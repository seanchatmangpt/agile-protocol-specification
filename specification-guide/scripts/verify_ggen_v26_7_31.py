...
REQUIRED_PHRASES = [
    "ontology-to-artifact manufacturing system",
    "HAND_CODED_GENERATED_OUTPUT",
    "parse -> route -> admit/refuse",
    "DuplicateReplay",
...
def verify() -> tuple[dict, int]:
    ...
    if data:
        ...
        if authority.get("canonical_authority") != "ontology":
            failures.append({"code": "ONTOLOGY_AUTHORITY_MISSING"})
        if authority.get("generated_outputs_are_authority") is not False:
            failures.append({"code": "GENERATED_OUTPUT_AUTHORITY_VIOLATION"})
        if authority.get("hand_edit_generated_outputs") != "REFUSED":
            failures.append({"code": "HAND_EDIT_REFUSAL_MISSING"})
        if authority.get("second_sync_expected") != "BYTE_IDENTICAL":
            failures.append({"code": "SECOND_SYNC_CONTRACT_MISSING"})

        actuation = data.get("actuation", {})
        if actuation.get("direct_actuation_allowed") is not False:
            failures.append({"code": "DIRECT_ACTUATION_ALLOWED"})
        if actuation.get("exclusive_path") != "BRCE":
            failures.append({"code": "BRCE_PATH_MISSING"})
        if actuation.get("invariant") != "Zero unreceipted actuation":
            failures.append({"code": "ACTUATION_INVARIANT_MISMATCH"})

        receipt = data.get("receipt", {})
        if receipt.get("ggen_algorithm") != "BLAKE3":
            failures.append({"code": "BLAKE3_REQUIRED"})
        if receipt.get("independent_replay_required") is not True:
            failures.append({"code": "INDEPENDENT_REPLAY_REQUIRED"})

        replay = data.get("replay", {})
        if replay.get("refusal_precedence") != ["DuplicateReplay", "ChainPositionViolation"]:
            failures.append({"code": "REPLAY_PRECEDENCE_MISMATCH"})
        if replay.get("refusal_preserves_state") is not True:
            failures.append({"code": "REFUSAL_STATE_SAFETY_MISSING"})

        blocks = data.get("building_blocks", {})
        if blocks.get("canonical_kernel_count") != 1:
            failures.append({"code": "CANONICAL_KERNEL_CARDINALITY"})
        if blocks.get("lifecycle_separate_from_standing") is not True:
            failures.append({"code": "LIFECYCLE_STANDING_COLLAPSED"})
        if blocks.get("profile_conflict") != "TYPED_REFUSAL":
            failures.append({"code": "PROFILE_CONFLICT_REFUSAL_MISSING"})

        self_play = data.get("self_play", {})
        if self_play.get("llm_calls_allowed") is not False:
            failures.append({"code": "LLM_SELF_PLAY_NOT_REFUSED"})
        if self_play.get("actors") != "EXPLICIT_STATE_MACHINES":
            failures.append({"code": "ACTOR_STATE_MACHINE_MISSING"})

        standards = data.get("standards", [])
        ids = [item.get("id") for item in standards]
        if len(ids) != len(set(ids)):
            failures.append({"code": "DUPLICATE_STANDARD_IDS"})
        missing = sorted(REQUIRED_STANDARD_IDS - set(ids))
        extra = sorted(set(ids) - REQUIRED_STANDARD_IDS)
        if missing:
            failures.append({"code": "REQUIRED_STANDARDS_MISSING", "ids": missing})
        if extra:
            failures.append({"code": "UNADMITTED_STANDARDS_PRESENT", "ids": extra})
        for index, item in enumerate(standards):
            for key in ["id", "title", "law", "admission", "obligations", "exclusions", "falsifier", "source_evidence"]:
                if not item.get(key):
                    failures.append({"code": "STANDARD_FIELD_MISSING", "index": index, "field": key})
            if item.get("admission") != "ADMITTED":
                failures.append({"code": "STANDARD_NOT_ADMITTED", "id": item.get("id")})
            if len(item.get("obligations", [])) < 2:
                failures.append({"code": "OBLIGATION_SET_INCOMPLETE", "id": item.get("id")})
            if not item.get("exclusions"):
                failures.append({"code": "EXCLUSION_SET_MISSING", "id": item.get("id")})

    if chapter:
        words = len(re.findall(r"\b[\w'–-]+\b", chapter))
        if words < 1800:
            failures.append({"code": "CHAPTER_NOT_SUBSTANTIVE", "words": words, "minimum": 1800})
        if chapter.count("\n# ") != 0 or not chapter.startswith("# "):
            failures.append({"code": "H1_CARDINALITY"})
        chapter_folded = chapter.casefold()
        for phrase in REQUIRED_PHRASES:
            if phrase.casefold() not in chapter_folded:
                failures.append({"code": "CHAPTER_INVARIANT_MISSING", "phrase": phrase})
        if "v26_7_30/16_ggen_manufacturing_standard.md" not in summary:
    ...
    report = {
        ...
        "standing": standing,
        "metrics": {
            "standards": len(data.get("standards", [])) if data else 0,
...
if __name__ == "__main__":
    ...
    print(json.dumps({
        ...
        "standards": result["metrics"]["standards"],
        "chapter_words": result["metrics"]["chapter_words"],
